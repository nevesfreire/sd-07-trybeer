...
const getOrderDetails = async (id) => {
  const [result] = await connection.execute(`SELECT s.id AS sale_number, s.status AS status,
    s.sale_date AS sale_date, sp.quantity AS quantity, p.name AS product_name,
     p.price AS  price FROM Trybeer.sales AS s 
     INNER JOIN Trybeer.sales_products AS sp ON s.id = sp.sale_id 
     INNER JOIN Trybeer.products AS p ON p.id = sp.product_id WHERE s.id=?`, [id]);

  return {
      saleNumber: result[0].sale_number,
