...
const createCheckout = async (ObjParams) => {
  ...
  for (let i = 0; i < cart.length; i++) {
    connection.execute('INSERT INTO sales_products (sale_id, product_id, quantity) VALUES (?, ?, ?)', [checkout.insertId, cart[i].id, cart[i].quantity]);
  }
  return { message: 'Pedido Enviado', checkout };
};

module.exports = {
  createCheckout, getAllSales, getSaleById
};
